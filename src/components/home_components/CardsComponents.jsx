...
const CardsComponents = (props) => {
  ...
  const [showFullContent, setShowFullContent] = useState(false) // New state variable
  ...
  const conditionContent = props.data.content.length > 75
  const cutContent = props.data.content.slice(0, 50)
          ...
          >
            <Card.Body>
              ...
              <Card.Text className="mt-3" style={{ marginBottom: '10px' }}>
                {showFullContent ? props.data.content : cutContent}
                {conditionContent && (
                  <span
